/**
 * Utility to check for and load saved measurements from surgical_case directory
 */
...
export async function checkSavedMeasurements(studyInstanceUID, seriesInstanceUID = null) {
  try {
    ...
    const response = await fetch(
      `http://localhost:3001/api/surgical-cases/list-csv?${params.toString()}`
    );
    ...
    console.warn('⚠️ Could not check for saved measurements:', error.message);
    return [];
  }
}
...
export async function loadMeasurementCSV(studyInstanceUID, seriesInstanceUID, filename) {
  try {
    ...
    const response = await fetch(
      `http://localhost:3001/api/surgical-cases/get-csv?${params.toString()}`
    );
    ...
    actions: [
      {
        ...
        text: 'Load',
        onClick: () => {
          console.log('📂 User chose to load saved measurements');
          onLoad(savedFiles);
        },
      },
      {
        ...
        text: 'Skip',
        onClick: () => {
          console.log('⏭️ User chose to skip loading measurements');
        },
      },
    ],
  });
